...
| --- | --- | --- |
| `server` | Select an existing connection by server URL. If no saved connection matches and the value is an HTTP(S) URL, connect to it. | URL string |
...
Supported JSON fields are `url`, `name`, `transportType`, `connectionType`, `customHeaders`, `auth`, `requestTimeout`, `resetTimeoutOnProgress`, and `maxTotalTimeout`.
