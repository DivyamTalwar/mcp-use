...
| `update <id-or-slug>` | Update server configuration. | `--branch <name>`, `--name <name>`, `--build-command <cmd>`, `--start-command <cmd>`, `--description <text>`, `--org <slug-or-id>` |
...
- Pass an empty string to `--build-command` or `--start-command` to clear an override.
