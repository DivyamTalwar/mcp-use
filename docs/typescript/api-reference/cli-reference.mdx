...
The `@mcp-use/cli` package installs the `mcp-use` executable. Use it to run and build TypeScript MCP servers, bundle MCP Apps widgets, deploy to Manufact Cloud, manage cloud resources, install agent skills, generate widget types, and drive MCP servers from the terminal.
...
| Config file | `~/.mcp-use/config.json` |
| Project link file | `.mcp-use/project.json` |
| Build manifest | `dist/mcp-use.json` |

## Command map

<CardGroup cols={2}>
  <Card title="Run locally" icon="flame" href="#dev---development-server">
    `mcp-use dev` starts a hot-reloading server with the Inspector mounted at `/inspector`.
  </Card>
  <Card title="Build and start" icon="box" href="#build---production-build">
    `mcp-use build` bundles server code and widgets. `mcp-use start` runs the built server.
  </Card>
  <Card title="Deploy" icon="cloud-upload" href="#deploy---cloud-deployment">
    `mcp-use deploy`, `mcp-use deployments`, and `mcp-use servers` manage Manufact Cloud deployments.
  </Card>
  <Card title="Client CLI" icon="terminal" href="/typescript/tooling/client-cli">
    `mcp-use client` calls tools, reads resources, gets prompts, handles OAuth, and captures widget screenshots.
  </Card>
  <Card title="Tunnels" icon="train-front-tunnel" href="/tunneling/index">
    `mcp-use dev --tunnel` and `mcp-use start --tunnel` expose a local server through a public URL.
  </Card>
  <Card title="Skills and types" icon="wand" href="#skills-add---install-ai-agent-skills">
    `mcp-use skills add` installs agent skills. `mcp-use generate-types` derives widget types from tool schemas.
  </Card>
</CardGroup>
...
| `--branch <name>`       | Deploy branch (also scopes `--env`/`--env-file` sync to that branch's preview env)   | Current git branch |
...
mcp-use servers update <id-or-slug> [--branch <name>] [--name <name>] [--build-command <cmd>] [--start-command <cmd>] [--description <text>] [--org <slug-or-id>]
mcp-use servers delete <server-id> [-y] [--org <slug-or-id>]   # alias: `rm`
```

| Subcommand | Description |
| ---------- | ----------- |
| `list` / `ls` | List servers for the active org or `--org` override |
| `get` | Show server details and recent deployments |
| `update` | Change the production branch, name, build command, start command, or description |
| `delete` / `rm` | Delete a server and its deployments; prompts unless `-y` is passed |

`servers update` accepts these options:

| Option | Description |
| ------ | ----------- |
| `--branch <name>` | New production branch; controls which branch triggers production deploys |
| `--name <name>` | Rename the server |
| `--build-command <cmd>` | Override the build command; pass an empty string to clear it |
| `--start-command <cmd>` | Override the start command; pass an empty string to clear it |
...
<Card title="Full Client CLI reference" icon="terminal" href="/typescript/tooling/client-cli">
...
```json
{
  ...
  "deploymentUrl": "https://my-server.run.mcp-use.com",
...
- [ServerConfig API reference](/typescript/api-reference/server/server-config) - Server configuration options
- [Deployment Guide](/typescript/server/deployment/mcp-use) - Deploy to Manufact Cloud
- [UI Widgets](/typescript/mcp-apps) - Build interactive widgets
- [Sessions API reference](/typescript/api-reference/server/sessions) - Session storage and stream manager APIs
